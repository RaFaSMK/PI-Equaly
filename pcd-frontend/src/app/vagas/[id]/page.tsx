...
export default function VagaDetalhePage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-zinc-900">{vaga.titulo}</h1>
      <p className="text-sm text-zinc-600">{vaga.empresa?.nomeFantasia}</p>
      <p className="whitespace-pre-wrap text-zinc-800">{vaga.descricao}</p>
      {vaga.escolaridade && (
        <div>
          <h3 className="text-sm font-medium">Escolaridade</h3>
          <p className="text-sm text-zinc-700">{vaga.escolaridade}</p>
        </div>
      )}
      ...
      {auth?.usuario?.tipo === "PCD" && (
        <div className="mt-6 space-y-2">
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-md border border-zinc-300 p-2 text-sm focus:ring-2 focus:ring-[#755fe3]"
          />
          <button
            onClick={candidatar}
